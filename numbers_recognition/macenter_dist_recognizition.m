...
[center_class,test_num,covmatric]=macenter_dist_pro(sample,evalResponse1,evalResponse2);
d=[];
center_class=uint8(center_class);
% test_num=double(test_num);
covmatric1=inv(covmatric);
for i=1:10
    d=[d,single((test_num-center_class(:,i)))'*single(covmatric1)*single((test_num-center_class(:,i)))];
end
[a,i]=min(d);
i=i-1;
